...
llists=$(ls ../../htdocs/deformations/*.param)
let i=0
...
cat << EOM
<HTML>
<HEAD><TITLE>Delete a list of subfaults</TITLE>
...
function Finalize(theSelFrom)
{
  ...
  if(str == "")
    ...
    window.open('/cgi-bin/priv/delete_faults_final.cgi'+'?'+str,'toolbar=no,location=no,directories=no,status=no,menubar=no,width=500,height=700');
}
